...
class Client(Base):
    ...
    async def authorize(self, email: EmailAddress, options: Optional[dict] = None) -> None:
    ...
    async def upload_file(self, file: BlobLike, options: Optional[UploadFileOptions] = None) -> UnknownLink:
    ...
    async def upload_directory(self, files: List[FileLike], 
                             options: Optional[UploadDirectoryOptions] = None) -> UnknownLink:
    ...
    async def upload_car(self, car: BlobLike, options: Optional[UploadOptions] = None) -> UnknownLink:
        """Upload a CAR file"""
        conf = await self._invocation_config([
            'space/blob/add',
            'space/index/add',
        ...
        if options is None:
            options = {
                'abilities': [
                    ...
                    'filecoin/accept', 'filecoin/submit'
                ],
        ...
        try:
            ...
            delegation_data = await self.agent.delegate({
                **options,
                'abilities': abilities,
                'audience': {
                    'did': lambda: DIDMailto.from_email(DIDMailto.email(delegate_email))
                },
    ...
    def delegations(self, caps: Optional[List[Capability]] = None) -> List['AgentDelegation']:
    ...
    async def revoke_delegation(
        self,
        delegation_cid: bytes,
        options: dict = None
    ) -> None:
        """Revoke a delegation"""
        return await self._agent.revoke(
            delegation_cid,
            {'proofs': options.get('proofs') if options else None}
        )

    async def remove(
        self,
        content_cid: UnknownLink,
        options: dict = None
    ) -> None:
    ...
    async def _remove_shard(self, shard: dict) -> None:
        ...
        try:
            ...
            if getattr(e, 'cause', None) and getattr(e.cause, 'name') != 'StoreItemNotFound':
