...
class Nullable(API[Optional[O], I, API[O, I, Any]]):
    ...
    def read_with(self, input_value: I, reader: API[O, I, Any]) -> Result:
        ...
        if result.error:
            return Result(error=UnionError([
                result.error,
                TypeError_("null", input_value)
            ]))
        return result
...
class UnionError(SchemaError):
    ...
    def __init__(self, causes: List[SchemaError]):
        ...
        super().__init__(
            f"Value does not match any type of the union:\n{messages}"
        )
...
class FieldError(SchemaError):
    ...
    def __init__(self, key: str, cause: SchemaError):
        ...
        self.cause = cause
        super().__init__(
            f"Object contains invalid field '{key}':\n  - {cause.message}"
        )
...
def dictionary(value_schema: API[V, I, Any], key_schema: Optional[API[str, str, Any]] = None) -> Dictionary[V, I]:
    return Dictionary({
        'value': value_schema,
        'key': key_schema or String()
    })
